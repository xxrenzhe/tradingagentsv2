...
def build_rule_prompt(*, segment: Segment, memory_packet: dict[str, Any]) -> str:
    return (
        ...
        "You may also use reproducible TradingView-style features such as macd_hist, adx_14, "
        "plus_di_14, minus_di_14, cci_20, stoch_rsi_k, donchian_20_position, keltner_position, "
        "supertrend_direction, vfi_130, vfi_hist, vfi_cross_up/down, vfi_zero_cross_up/down, "
        "eqh_signal, eql_signal, liquidity_pool_signal, range_100_position, demand_zone_retest, "
        "supply_zone_retest, and order_block_retest_signal when they appear in SEGMENT_FEATURES. "
        ...
        "Do not put setup thresholds only in invalid_if; every tradeable threshold must appear in "
        "entry_conditions as {feature, operator, value}. invalid_if is only for extra failure guards. "
        "For feature-to-feature comparisons, set value to the other exact feature name, for example "
        "{\"feature\":\"plus_di_14\",\"operator\":\">\",\"value\":\"minus_di_14\"}.\n\n"
