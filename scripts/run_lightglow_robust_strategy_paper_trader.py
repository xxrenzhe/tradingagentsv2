...
def main() -> int:
    load_project_env()
    parser = argparse.ArgumentParser(description="Run the selected robust Lightglow strategy through MNQ paper validation.")
    parser.add_argument("--trades", default=".tmp/nq-lightglow-robust-strategy-trades.csv")
    parser.add_argument("--symbol", default="MNQ")
    parser.add_argument("--trade-date", default=None)
    parser.add_argument("--row-index", type=int, default=None)
    parser.add_argument("--state-path", default=".tmp/nq-lightglow-robust-paper-runner-state.json")
    parser.add_argument("--contract-month", default="202606")
    parser.add_argument("--account", default=None)
    parser.add_argument("--quantity", type=int, default=1)
    parser.add_argument("--submit", action="store_true", help="Submit to IBKR paper. Blocked by default until timed exit execution is available.")
    parser.add_argument(
        "--allow-entry-only-submit",
        action="store_true",
        help="Allow entry-only submit for controlled operator tests. This does not implement the 3-minute time exit.",
    )
    ...
    if args.submit and not args.allow_entry_only_submit:
        result = {
            ...
            "details": "This strategy must exit after one 3-minute bar and has no stop-loss/take-profit bracket.",
        }
    ...
    if args.daemon:
        result = run_adaptive_portfolio_paper_daemon(
            config=PaperDaemonConfig(
                ...
                portfolio_rule=LIGHTGLOW_ROBUST_STRATEGY_ID,
    ...
    result["strategy_lock"] = {
        ...
        "symbol": args.symbol.upper(),
        "quantity": args.quantity,
        "exit_policy": "time_exit_after_one_3m_bar",
        "bracket": "disabled",
    }
    print(json.dumps(result, indent=2, sort_keys=True, default=str))
    if args.daemon:
